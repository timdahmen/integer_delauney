...
During profiling of the incremental insertion pipeline, the duplicate seed validation step was identified as one of the largest CPU-side bottlenecks, consuming approximately **2000 ms** for large insertion batches.

Originally, duplicate detection compared every new seed against every other seed using nested loops:
...
This approach has a time complexity of **O(k²)**, meaning the execution time grows quadratically with the number of inserted seeds.
...
The hash table provides average **O(1)** insertion and lookup, reducing the overall duplicate check to **O(k)**.

Calling `reserve(k)` allocates sufficient buckets beforehand, avoiding costly rehash operations while inserting the batch.

### Result

Replacing the quadratic duplicate search with hash-based lookup removes the bottleneck for large insertion batches and scales linearly with the number of seeds.

| Implementation | Time Complexity |
|----------------|-----------------|
| Nested loops | O(k²) |
| `std::unordered_set` | O(k) |

    ...
    int32_t *__restrict__ counter)
{
    ...
    if (x >= 1 && y <= H - 2)
        try_register(x, y, idx(x - 1, y), idx(x, y), idx(x, y + 1));
    if (x <= W - 2 && y <= H - 2)
        try_register(x, y, idx(x, y), idx(x + 1, y), idx(x, y + 1));
    if (x <= W - 2 && y >= 1)
        try_register(x, y, idx(x, y), idx(x + 1, y), idx(x, y - 1));
    if (x >= 1 && y >= 1)
        try_register(x, y, idx(x - 1, y), idx(x, y), idx(x, y - 1));
...
__global__ void split_seed_distance_grid_kernel(
    ...
    TriTimings *timings)
{
    ...
    auto make_event = [](cudaEvent_t *e)
    { cudaEventCreate(e); };
    auto record = [](cudaEvent_t e)
    { cudaEventRecord(e); };

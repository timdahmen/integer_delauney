...
__device__ __forceinline__ bool point_in_triangle(float px, float py,
...
__global__ void voronoi_step_kernel(
...
__global__ void find_triangle_seeds_kernel(
    ...
    const int32_t *__restrict__ mask) // nullptr → all pixels
{
    ...
    {
        if (a == UNDEF_SEED || b == UNDEF_SEED || c == UNDEF_SEED)
            return;
        if (a == b || a == c || b == c)
            return;
    ...
    if (x >= 1 && y <= H - 2)
        try_reg(x, y, sid(x - 1, y), sid(x, y), sid(x, y + 1));
    if (x <= W - 2 && y <= H - 2)
        try_reg(x, y, sid(x, y), sid(x + 1, y), sid(x, y + 1));
    if (x <= W - 2 && y >= 1)
        try_reg(x, y, sid(x, y), sid(x + 1, y), sid(x, y - 1));
    if (x >= 1 && y >= 1)
        try_reg(x, y, sid(x - 1, y), sid(x, y), sid(x, y - 1));
...
__global__ void remap_tgrid_kernel(int32_t *__restrict__ t_grid, int N,
...
__global__ void assign_triangles_kernel(
